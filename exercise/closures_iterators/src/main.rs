...
fn main() {
    ...
    for (item1, item2) in pairs2 {
        println!("{:?}", (item1 + 1, item2));
    }

    let mut numbers2 = vec![1, 2, 3, 4];
    let multiplied = numbers2
        .iter_mut()
        .map(|number| *number * 3) // multiply the value by 3 via the mutable reference x
        .collect::<Vec<_>>();
    println!("{:?}", multiplied); // should print [3, 6, 9, 12]
}
